...
library(lmtest)
library(nlme)
###La opción simplificada es
m_ar1 <- gls(tsc3 ~ -1 + tsc1, correlation = corAR1())
summary(m_ar1)

m_ma1 <- gls(tsc3 ~ -1 + tsc1, correlation = corARMA(q = 1))
#m_ma1 <- gls(tsc3 ~ tsc1, correlation = corARMA(q = 1))
#m_ma1 <- gls(tsc3 ~ -1 + tsc1, correlation = corARMA(q = 1),control = glsControl(opt = "optim"))
# m_ma1 <- gls(tsc3 ~ -1 + tsc1, correlation = corARMA(q = 1),control = glsControl(msMaxIter = 1000, msTol = 1e-4))
summary(m_ma1)
...
Note que este resultado es análogo al obtenido al hacer la regresión con errores ARMA de la función Arima. Compare los resultados con el de la salida de lm.
