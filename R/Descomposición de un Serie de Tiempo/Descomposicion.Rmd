...
fltr
## estimate of trend
lAirPass.trend <- stats::filter(lAirPass, filter = fltr, method = "convo", sides = 1)
## plot the trend
#x11()
par(mar=c(2,2,2,2))
par(mfrow=c(2,1))
...
```{r Analisis de la serie sin tendencia}
library(tibble)
monthplot(lAirPass.1T)
spectrum(na.omit(lAirPass.1T),log='no')
library(astsa)
Airpass.per = astsa::mvspec(na.omit(lAirPass.1T), log="no")
Airpass.per = astsa::mvspec(na.omit(lAirPass.1T), log="no")
library(biwavelet)
na.omit(lAirPass.1T)

lAirPass_wavelets<-cbind(seq(1:length(na.omit(lAirPass.1T))),as.numeric(na.omit(lAirPass.1T)))


wt<-biwavelet::wt(lAirPass_wavelets)
...
library(lubridate)
library(ggplot2)
lAirPass_tsibble=as_tsibble(na.omit(lAirPass.1T))

lAirPass_tsibble%>%mutate(month=factor(lubridate::month(index),levels=1:12,label=month.abb))%>%
...
HWAP_train$beta
HWAP_train$gamma
###Observación: Note que esos son las estimaciones de los parámetros de suavizamiento. Se puede también hacer una grilla de valores para explorar si hay unos valores mejores.
...
for(i in 1:(ntest))
{
  ...
  refit=stats::HoltWinters(x,seasonal="additive",alpha=grilla_suav[1,1],beta=grilla_suav[1,2],gamma=grilla_suav[1,3])
