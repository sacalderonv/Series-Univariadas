...
#fUnitRoots::urkpssTest(drift,type = "tau",doplot = FALSE)##Estacionariedad en niveles. La nula es I(0) 
kpss.drfit<-urca::ur.kpss(drift,type = "tau") # type="mu" indica que el proceso bajo Ho es estacionario alrededor de una nivel fijo(ro), mientras que "tau" es estacionario alrededor de una tendencia determinística. lags tiene 3 formas de establecerse "nil" implica l=0 y no usa retardos de la covarianza, "long" usa mas retardos de la covarianza, mientras "short" usa menos.
kpss.drfit@cval
###Otro Ejemplo
...
kpss.trend<-urca::ur.kpss(trend.estacionario,type = "mu",lags = "short") #
kpss.trend@cval
```
...
kpss.air<-urca::ur.kpss(logAirP,type = "mu",lags = "short") #
kpss.air@cval
kpss.air
...
ar(dlogAirPass)
aTSA::adf.test(dlogAirPass,nlag = 16)
summary(ur.df(dlogAirPass,type="none",lags = 14))

kpss.air_diff<-urca::ur.kpss(dlogAirPass,type = "mu",lags = "short") #
kpss.air_diff@cval
kpss.air_diff
...
#####Fase de Pronósticos
pronosticos12=forecast::forecast(AjusteArima12,h=12,level=0.95,biasadj=TRUE)
plot(pronosticos12)
```



```{r ajuste de la estacionalidad con componentes Fourier, Dummy y SARIMA}
...
ajuste_final %>%
    forecast::forecast(h = "12months",biasadj=TRUE) %>%
  autoplot( level = 95) +
...
`SARIMA`=ARIMA(log(value)~1+pdq(1,1,0)+PDQ(1,1,0)))
...
ajuste_final_models %>%
  forecast::forecast(h = "2years",biasadj=TRUE) %>%
