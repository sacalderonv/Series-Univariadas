...
La serie es medida mensualmente, es decir, presenta una frecuencia de
medición 12, es decir, cada 12 periodos volvemos a ver el mismo mes. Qué
...
library(readxl)
library(tidyverse)
setwd("~/Documents/Documentos - iMac de Sergio/Documentos - iMac de Sergio/Documentos iMac Sergio/Cursos/Time Series/2026_II_TeoriaSeriesUnivariadas")
#DesempleoyEmpleo <- read_excel("DesempleoyEmpleo.xlsx", range="A6:C269")
Desempleo13 <- read_excel("BaseSeriesDesempleo.xlsx",sheet = "Desempleo")
str(Desempleo13)
...
library(zoo)
library(xts)
Fechas=as.yearmon(Desempleo13$Fecha)
Desempleo_Col13_xts=xts(x = Desempleo13$TsaDesem13Ciud,frequency = 12,order.by = Fechas)
ts_info(Desempleo_Col13_xts)
plot(Desempleo_Col13_xts)
```

```{r grafico Studio Desemplo Col}
ts_plot(Desempleo_Col13_xts,
...
df_desempleo13=data.frame(Desempleo=Desempleo13$TsaDesem13Ciud,Fecha=Desempleo13$Fecha)
tbl_desempleo13=tibble(df_desempleo13)
tbl_desempleo13_format_fecha=tbl_desempleo13
tbl_desempleo13_format_fecha$Fecha=yearmonth(tbl_desempleo13_format_fecha$Fecha)
###El tipo de fechas debe ser alguno que reconozca tsibble

tsbl_desempleo13=as_tsibble(tbl_desempleo13_format_fecha,index=Fecha)   ####La fecha en tsibble es importante

##Gráfica de tsibble
autoplot(tsbl_desempleo13,Desempleo)+labs(tittle="Serie de Desempleo Colombia Mensual",y="Tasa de Desempleo")
tbl_desempleo13$Fecha<-as.Date(zoo::as.yearmon(tbl_desempleo13$Fecha))
tbl_desempleo13
###Gráfica timetk
tbl_desempleo13%>%plot_time_series(.value=Desempleo,.date_var=Fecha)
...
Simule unos datos, crear un objeto con periodicidad s=12, y extraer la
descomposición, qué puede observar?
...
tibble_chicken%>%mutate(Pollo_ajus=smooth_vec(Pollo,span = 0.25, degree = 2))
...
# Continuación estimación tendencia por suavizamiento
...
# Diferenciación como estrategia para eliminar la tendencia
...
lines(x, col=4); abline(h=0, col=4, lty=2); abline(a=0, b=.1, lty=2)
...
#
car::boxCox(AirPassengers~1)###Me entrega una gráfica
car::powerTransform(AirPassengers~1)

plot(forecast::BoxCox(AirPassengers,lambda=0.0))###
...
forecast::BoxCox.lambda(lAirPass, method = "loglik", lower = -1, upper = 2)

car::boxCox(lAirPass~1)###Me entrega una gráfica
...
car::boxCox(AirPassengers~1,family = "yjPower")
car::powerTransform(AirPassengers~1,family = "yjPower")
...
Ejercicio: Use la serie *varve* del paquete **astsa** para chequear si
es necesario hacer transformación Box-Cox.

## Gráficas de Retardos Para Series Estacionarias o al menos sin tendencia
...
asociada al soi)**, número de nuevos peces.

También chequearemos la serie consumo mensual de gas natural en EE.
UU.(USgas) medido en Billones de pies cúbicos. Esto permite chequear si
hay posibles relaciones no-lineales.
...
La correlación cruzada permite medir la dependencia lineal temporal
...
Sin embargo se debe pre-blanquear als dos series para eliminar el efecto
de las dos series de tiempo
...
La autocorrelación de distancia también permite detectar relaciones no
lineales
...
- El primer ciclo ocurrió entre 1990 y 2000, que estuvo cerca de un
  ciclo de 11 años.
- El segundo ciclo comenzó en 2000 y finalizó en 2007, que fue un ciclo
  de 7 años.
- Un tercer ciclo, que comenzó en 2007 y continúa a mayo de 2019 aún no
  se ha completado, lo que significa que se ha prolongado durante más de
  12 años.
...
- Horaria: Este es probablemente el principal patrón estacional de la
  serie, ya que existe una relación directa entre la demanda de energía
  eléctrica y la hora del día (hay alta demanda durante el día y baja
  demanda durante la noche o al contrario).
- Día de la semana: La demanda de electricidad a lo largo del día se
  deriva, potencialmente, del día de la semana. Tendría sentido esperar
  un alto consumo durante los días laborables y una menor tasa de
  consumo durante el fin de semana.
- Mensual: como los patrones climáticos varían a lo largo del año, la
  cantidad de luz del día y otros factores estacionales podrían afectar
...
- Hay poca demanda durante la noche (entre la medianoche y las 6 a.m.) y
  una alta demanda entre las horas de la mañana y la tarde.
...
- La relativamente baja desviación estándar de la demanda promedio
  ...
  subestacional durante esas horas además de la estacionalidad horaria.
  Esto debería tener sentido, ya que son horas de sueño normales y, por
  lo tanto, en promedio, la demanda es razonablemente la misma durante
  los días de semana.

- Por otro lado, la alta desviación estándar a lo largo de las horas de
  alta demanda podría indicar que la demanda se distribuye de manera
  diferente en diferentes vistas de periodicidad (como día de la semana
  o mes del año).
...
```{r subseries}
library(ggtime)
UKgrid_tstbl%>%ggtime::gg_subseries(ND,period=24)
...
```{r Wavelet UKGrid,warning=FALSE, message=FALSE,results='hide'}
which(is.na(UKgrid_tstbl), arr.ind = TRUE)

series_wavelets_uk<-cbind(seq(1:length(UKgrid_tstbl$ND)),na.approx(UKgrid_tstbl$ND))



wt<-biwavelet::wt(series_wavelets_uk)
...
**Como estimar la componente estacional?**

```{r estimar componete estacional1}
...
Ahora definimos los tres ciclos: el diario (24), el semanal
(168**=24x7**) y el anual (**8766=365.25x24**). Usaremos `K` (número de
armónicos) para controlar qué tan "curvada" es la estimación.
...
Ajuste de cada componente via splines.
...
Que pasa si modelamos el mes pero que no sea tan suave?
...
# Mas sobre Suavizamiento en el contexto de Series de Tiempo
