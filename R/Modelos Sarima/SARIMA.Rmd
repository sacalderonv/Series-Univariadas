...
ar(lAirpassengers)
aTSA::adf.test(lAirpassengers,nlag = 14)
...
fUnitRoots::adfTest(lAirpassengers,lags=12,type='ct')  ####Puede también indicar
...
aTSA::adf.test(dlAirpassengers,nlag = 15)  ###No se debe diferenciar más###

fUnitRoots::adfTest(dlAirpassengers,lags = 14,type='nc') 

df_ur=urca::ur.df(dlAirpassengers, lags = 14)
summary(df_ur)
...
#En resumen podríamos proponer max q=3, Q=1, p=9, P=1
#SARIMA(p=0,d=1,q=3)x(P=0,D=1,Q=1)s=12 con drfit?
```
...
## Tarea: Ajustar un modelo SARIMA pero D=1!

## Pronóstico

```{r Pronostico}
#x11()
Pronosticos=forecast::forecast(modelo,h=12,level=0.95,biasadj=TRUE)
plot(Pronosticos)
...
esta_param_modelo<-coef(fitmodelo)
h <- 2
train <- window(AirPassengers,start=c(1949,01),end=c(1959,12))
test <- window(AirPassengers,start=c(1960,01),end=c(1960,12))
n <- length(test) - h + 1
fitmodelo <- Arima(AirPassengers, c(0, 1, 1),seasonal = list(order = c(0, 1, 1), period = 12),lambda = 0,fixed=esta_param_modelo)
...
Ajuste_tsibble_Pass %>%
  forecast::forecast(h=12,biasadj=TRUE) %>%
